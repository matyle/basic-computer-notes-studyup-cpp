...
| Age | 推算资源创建经过时间（代理服务器必须加这个响应字段） |  |
| ETag | 资源的匹配信息(URI相同，但是资源不同，例如网站的语言) |  |
...
| Content-Encoding | 实体主体适用的编码方式(gzip,compress,deflate,identity) | Content-Encoding:gzip |
| Content-Language | 实体主体的自然语言 | Content-Language:zh-CN |
| Content-Length | 实体主体部分的大小 | Content-Length:15000 |
| Content-Location | 替代对应资源的 URI | Content-Location:http://www.baidu.com |
| Content-MD5 | 实体主体的MD5算法编码，然后利用Base64编码后结果写入该字段 |  |
| Content-Range | 实体主体的位置范围 | Content-Range:bytes 5001-10000/10000 |
| Content-Type | 实体主体的媒体类型 |  |
| Expires | 实体主体缓存过期的日期时间 |  |
